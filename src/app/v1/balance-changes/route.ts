...
// balance_changes = the full balance ledger with a reason (GAS_BUY, TRANSFER,
// REWARD, …): every wei moved, attributed. Data RPC can't produce.
...
export async function GET(req: Request) {
  try {
    ...
    const changes = rows.map((r) => ({
      ...
      old_value: r.old_value as string, // decimal-string wei, big-int safe
    ...
    return NextResponse.json(
      { dataset: DATASET, count: changes.length, balance_changes: changes },
